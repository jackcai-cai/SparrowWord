...
final class AppState: ObservableObject {
    ...
    func lookupCurrentTerm() {
        ...
        let usesProgressiveLocalLookup = shouldUseProgressiveLocalLookup(
            draft: draftSnapshot,
            settings: settingsSnapshot,
        ...
        currentLookupTask = Task { [weak self] in
            ...
            let result = await self.lookupService.generateLookup(
                term: draftSnapshot.trimmedTerm,
                kind: draftSnapshot.kind,
                settings: settingsSnapshot,
                apiKey: apiKeySnapshot
    ...
    var sentenceEngineLookupStatusText: String {
        if settings.offlineResources.sentenceEngineStatus == .failed && !hasAttemptedSentenceLookupInSession {
            return "待重试"
        }
    ...
    func persistLookupHistory() {
        do {
            try storage.saveLookupHistory(lookupHistory)
        } catch {
            statusMessage = "查词历史保存失败：\(error.localizedDescription)"
        }
    }

    func persistReviewHistory() {
        do {
            try storage.saveReviewHistory(reviewHistory)
        } catch {
            statusMessage = "复习历史保存失败：\(error.localizedDescription)"
        }
    }
    ...
    private func persistCaptureDrafts() {
        do {
            try storage.saveCaptureDrafts(savedCaptureDrafts)
        } catch {
            statusMessage = "保存 Quick Capture 草稿失败：\(error.localizedDescription)"
        }
    }

    private func persistTrashItems() {
        do {
            try storage.saveTrashItems(trashItems)
        } catch {
            statusMessage = "回收站保存失败：\(error.localizedDescription)"
        }
    }
    ...
    private func shouldUseProgressiveLocalLookup(
        draft: LookupDraft,
        settings _: AppSettings,
        apiKey _: String
    ) -> Bool {
        guard draft.kind != .sentence else {
            return false
        }

        guard !containsChineseCharacters(in: draft.trimmedTerm) else {
            return false
        }
    ...
    private func previewLookupRecord(
        originalQuery: String,
        kind: EntryKind,
    ...
    private static func normalizedLoadedEntry(_ entry: VocabEntry) -> VocabEntry {
    ...
    private static func normalizedLoadedTrashItem(_ item: TrashItem) -> TrashItem {
    ...
    private static func normalizedLoadedCaptureDraft(_ draft: CaptureDraft) -> CaptureDraft {
        var normalizedDraft = draft
        normalizedDraft.sanitizeSelections()
        return normalizedDraft
    }

    private static func normalizedLoadedLookupHistoryRecord(_ record: LookupHistoryRecord) -> LookupHistoryRecord {
    ...
    private static func normalizedLoadedReviewHistory(_ records: [ReviewHistoryRecord]) -> [ReviewHistoryRecord] {
    ...
    private static func normalizedLegacyLookupStatusMessage(_ message: String?) -> String? {
    ...
    private static func captureDraftFromLegacyInboxEntry(_ entry: VocabEntry) -> CaptureDraft {
        captureDraft(from: entry)
    }

    private static func captureDraft(from entry: VocabEntry) -> CaptureDraft {
    ...
    private static func migrateLegacyInboxState(
        entries: [VocabEntry],
    ...
    private static func shouldKeepLegacyInboxEntryAsDraft(_ entry: VocabEntry) -> Bool {
    ...
    private static func upsertMigratedCaptureDraft(
    ...
    private static func mergedLibraryEntry(primary: VocabEntry, secondary: VocabEntry) -> VocabEntry {
    ...
    private static func captureDraftStorageKey(for term: String, kind: EntryKind) -> String {
        "\(kind.rawValue)::\(term.trimmingCharacters(in: .whitespacesAndNewlines).lowercased())"
    }

    private static func containsChineseCharactersForLegacyMigration(_ text: String) -> Bool {
        text.unicodeScalars.contains { scalar in
            switch scalar.value {
            case 0x3400...0x4DBF, 0x4E00...0x9FFF, 0xF900...0xFAFF:
                return true
    ...
    private static func hasPendingOfflineLexiconEnrichment(
        entries: [VocabEntry],
    ...
    private static func enrichedLoadedOfflineData(
        entries: [VocabEntry],
        lookupHistory: [LookupHistoryRecord],
        manifest: OfflineResourceManifest
    ...
    private static func enrichedLoadedEntries(
        _ entries: [VocabEntry],
        manifest: OfflineResourceManifest
    ) -> [VocabEntry] {
    ...
    private static func enrichedLoadedLookupHistory(
        _ records: [LookupHistoryRecord],
        manifest: OfflineResourceManifest
    ) -> [LookupHistoryRecord] {
        guard !records.isEmpty else {
            return []
    ...
    private static func enrichedLoadedEntry(
        _ entry: VocabEntry,
        manifest: OfflineResourceManifest,
    ...
    private static func enrichedLoadedLookupHistoryRecord(
    ...
    private static func shouldAttemptOfflineLexiconEnrichment(
        term: String,
        kind: EntryKind,
    ...
    private static func shouldApplyOfflineLexiconEnrichment(
    ...
    private static func cachedOfflineLexiconPreview(
        term: String,
        kind: EntryKind,
        manifest: OfflineResourceManifest,
        cache: inout [String: OfflineLexiconCore?]
    ...
    private static func remappedSelectionIndexes(_ selectedMeanings: [String], in choices: [String]) -> [Int] {
    ...
    private static func captureMeaningCandidates(
        meaningGroups: [MeaningGroup],
    ...
    private static func meaningGroups(
    ...
    private static func mergedCaptureMeaningCandidates(
    ...
    private static func isLikelyEnglishLexiconTerm(_ term: String) -> Bool {
        let cleanedTerm = term.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !cleanedTerm.isEmpty else {
            return false
        }
    ...
    private static func normalizedPartOfSpeechKey(_ partOfSpeech: String) -> String {
        partOfSpeech
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
    }

    private static func normalizedChoiceEntries(_ choices: [String], maxCount: Int) -> [String] {
        Array(
            choices
    ...
    private static func mergedEntryMeaningChoices(
    ...
    private static func normalizedStoredMeaningKey(_ text: String) -> String {
        text
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
    }

    private static func normalizedLoadedMeaningGroups(
        kind: EntryKind,
        partOfSpeech: String,
        meaningGroups: [MeaningGroup],
        meanings: [String],
    ...
    private static func sanitizedStoredMeaningText(_ text: String) -> String {
        let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else {
            return ""
        }
    ...
    private static func expandedStoredMeaningTexts(from text: String) -> [String] {
    ...
    private static func isVerbInflectionMeaning(_ meaning: String) -> Bool {
    ...
    private static func repairedStoredMeaningPartOfSpeech(
        for meaning: String,
        fallbackPartOfSpeech: String,
    ...
    private static func containsChineseCharactersForStoredMeaning(_ text: String) -> Bool {
        text.unicodeScalars.contains { scalar in
            switch scalar.value {
            case 0x3400...0x4DBF, 0x4E00...0x9FFF, 0xF900...0xFAFF:
                return true
    ...
    private static func cleanedGeneratedExamples(
        _ examples: [String],
        term: String,
        sourceContext: String
    ) -> [String] {
    ...
    private static func isFallbackGeneratedExample(_ example: String, term: String) -> Bool {
    ...
    private static func fallbackGeneratedExamples(term _: String, sourceContext: String) -> [String] {
